...
import { VariantCard } from "@/components/tours/VariantCard";
...
export function CategorySection({
  ...
  return (
    <section
      id={category.slug}
      ref={sectionRef}
      className="scroll-mt-24 px-3 py-6 md:px-5 md:py-10"
    >
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 md:mt-6 md:grid-cols-2 md:gap-4">
        {tours?.map((tour, i) => (
          <VariantCard key={tour.slug} tour={tour} style={reveal.getAnimStyle(i)} />
        ))}
